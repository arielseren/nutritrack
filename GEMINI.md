...
│   │   ├── custom/
│   │   │   └── CustomFoodModal.tsx # יצירת מאכל חדש והזנת ערכים ל-100 גרם או הזנה ישירה
│   │   ├── plans/
│   │   │   ├── MealPlansModal.tsx# מתכנן שבועי (7 ימים) + תפריטים מוכנים + טאב "התפריטים שלי"
...
│   │   ├── guide/
│   │   │   └── UserGuideModal.tsx      # מדריך למשתמש אינטראקטיבי בתוך האפליקציה עם 8 פרקים וחיפוש מהיר
│   │   ├── profile/
│   │   │   └── ProfileSettingsModal.tsx # 5 כרטיסיות אקורדיון, לוח אימונים שבועי, ביומטרי וגיבויים
│   │   ├── auth/
...
3. **הזנה ישירה של קלוריות ומאקרו ללא חישוב 100 גרם (Direct Macro & Meal Logging):**
...
7. **בונה תפריטי תזונה ומתכנן שבועי (Weekly Meal Planner & Builder):**
   ...
   - **תבניות שבועיות מוכנות:** שיבוץ שבוע חיטוב (1,800), שבוע מסה (2,400) או שבוע ים-תיכוני (2,000).
   - **בונה תפריטים מותאמים אישית:** הגדרת שם, מטרה, הוספת מאכלים וכמויות גרמים מדויקות ל-4 ארוחות עם חישוב מאקרו דינמי בזמן אמת.

8. **סייקלינג קלוריות יומי והתאמה דינמית לסוג ועצימות האימון (Workout & Calorie Cycling):**
   - התאמה מדעית של יעד הקלוריות והפחמימות לכל יום בנפרד לפי סוג הפעילות:
     - 🛋️ **יום מנוחה והתאוששות:** מאזן בסיסי לשיקום השרירים.
     - 🏋️ **אימון כוח / פלג גוף עליון:** תוספת +250 קק"ל ופחמימות למילוי מאגרי גליקוגן.
     - 🔥 **אימון כבד / רגליים / עצימות גבוהה:** תוספת +450 קק"ל ודגש על פחמימות מוגברות לבניית שריר.
     - 🏃 **אירובי / ריצה / שחייה:** תוספת +350 קק"ל ואנרגיה זמינה.
     - ⚡ **אימון אינטרוולים / HIIT:** תוספת +400 קק"ל לשריפת אנרגיה מוגברת.
     - 🎯 **אימון מותאם אישית:** הזנת קלוריות שנשרפו באימון ומשך הזמן בדקות ידנית.
   - סרגל בחירה מהיר בלחיצה אחת ב-Dashboard וביומן הארוחות.
   - לוח אימונים שבועי מובנה בפרופיל עם תבניות מוכנות (AB 4 ימים, Push-Pull-Legs 5 ימים, Full-Body 3 ימים) המגדיר אוטומטית את סוג היום.

9. **בידוד נתונים ושמירה אישית מלאה לכל משתמש (User-Scoped Data Isolation):**
...
10. **אימות ביומטרי והתחברות מאובטחת (WebAuthn Fingerprint / FaceID):**
...
11. **מדריך למשתמש אינטראקטיבי בתוך האפליקציה (Interactive In-App User Guide):**
    - כפתור מדריך ייעודי בסרגל העליון (📖) ובמסך הפרופיל.
    - 8 פרקים מקיפים עם אקורדיונים, הסברים מפורטים, טיפים וחיפוש טקסטואלי מהיר בזמן אמת:
      1. הרשמה, פרופיל וחישוב יעדים מדעי (BMR & TDEE).
      2. הזנה ישירה ותיעוד מזון מהיר (ללא 100 גרם ושיטת כף היד).
      3. סייקלינג קלוריות יומי והתאמה לאימונים.
      4. מתכנן תפריט שבועי ורשימת קניות חכמה.
      5. בונה תפריטי תזונה מותאמים אישית.
      6. בידוד נתונים אישי ואבטחה ביומטרית (WebAuthn).
      7. התראות Web Push ותזכורות מותאמות אישית.
      8. התקנת האפליקציה בטלפון (PWA) ומצב לילה.
